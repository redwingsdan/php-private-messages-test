...
class DbHandler {
	...
	public function createUserTask($task, $status, $assignedTo){
		$stmt = $this->conn->prepare("INSERT INTO `tasks`(`task`, `status`, `assigned_to`) VALUES(?,3,?) ");
        $stmt->bind_param("ss", $task, $assignedTo);
        $stmt->execute();
        $stmt->store_result();
        $num_rows = $stmt->num_rows;
        $stmt->close();
     ...
     * @param String $order_id id of the order
     */
	public function getAllOrders3($user_id) {
	...
	public function updateOrder2($user_id, $order_id, $status, $assignedTo) {
		...
		$stmt = $this->conn->prepare("UPDATE users u set u.status = 2 WHERE u.id = ?");
		$stmt->bind_param("i", $ident);
		$stmt->execute();
		$stmt->close();
		
	    $stmt = $this->conn->prepare("UPDATE tasks t set t.target = ? WHERE t.user = ?");
	...
	public function updateOrder3($user_id, $order_id, $status, $assignedTo) {
	...
	if($status == "0"){
		$stmt = $this->conn->prepare("UPDATE tasks t set t.assigned_to = ?, t.target = NULL, t.user = ? WHERE t.id = ?");
		$stmt->bind_param("sii", $assignedTo, $ident, $order_id);
	}
	else{
	    $stmt = $this->conn->prepare("UPDATE tasks t set t.assigned_to = ?, t.target = ?, t.user = ? WHERE t.id = ?");
		$stmt->bind_param("siii", $assignedTo, $status2, $ident, $order_id);
	}
        $stmt->execute();
        $num_affected_rows = $stmt->affected_rows;
        $stmt->close();
    ...
    public function deleteOrder($user_id, $id) {   
	 ...
	 public function deleteOrder2($user_id, $id) {   
