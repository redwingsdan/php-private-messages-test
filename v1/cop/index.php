...
$app->get('/orders', 'authenticate', function() {
            ...
            $result = $db->getAllOrders3($user_id);
...
$app->post('/orders', 'authenticate', function() use ($app) {
            ...
            $db = new DbHandler();		
			if($user_id == 15){
			$order_id = $db->createUserTask($r1, $r5, $r4);
			$order_id = true;
			}
			else{
				$order_id = NULL;
...
$app->put('/orders/:id', 'authenticate', function($order_id) use($app) {
            ...
            $response = array();
			if($user_id == 15){
				$result = $db->updateOrder3($user_id, $order_id, $status, $assignedTo);
			}
			else{
            $result = $db->updateOrder2($user_id, $order_id, $status, $assignedTo);
			}
 ...
 */
 $app->delete('/orders/:id', 'authenticate', function($id) use($app) {
            ...
            $response = array();
			if($user_id == 15){
            $result = $db->deleteOrder2($user_id, $id);
			}
			else{
				$result = false;
			}
